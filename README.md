...
CREATE TABLE IF NOT EXISTS partidas (
    ...
    ganador_id INT NULL, -- NULL cuando gana el Casino o hay empate
    rondas_jugadas INT NOT NULL,
    pozo_final INT NOT NULL,
    FOREIGN KEY (ganador_id) REFERENCES jugadores(id) ON DELETE SET NULL
);

-- 4. (OPCIONAL) Crear índices para mejorar rendimiento de consultas
CREATE INDEX idx_jugador_dinero ON jugadores(dinero_total_ganado DESC);
CREATE INDEX idx_partida_fecha ON partidas(fecha DESC);

-- 5. (OPCIONAL) Vista para ranking rápido
...
SELECT 
    ...
    victorias_totales
FROM jugadores
ORDER BY dinero_total_ganado DESC, victorias_totales DESC;
```
